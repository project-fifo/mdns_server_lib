...
%% gen_fsm callbacks
-export([init/1, reading/2, handle_event/3,
...
%%--------------------------------------------------------------------
start_link(Handler) ->
    gen_fsm:start_link({local, ?SERVER}, ?MODULE, [Handler], []).
...
init([Handler]) ->
    ...
    {ok, Socket} = erlzmq:socket(Ctx, [rep, {active, false}]),
...
%% @end
%%--------------------------------------------------------------------
reading(_Event, #state{socket = Socket,
		       handler = Handler,
